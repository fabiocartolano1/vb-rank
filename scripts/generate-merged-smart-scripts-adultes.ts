...
const championnats = [
  { code: 'n3', championnatId: 'nationale-3-f', stateKeyClassement: 'n3-classement', stateKeyMatchs: 'n3-matchs', nom: 'Nationale 3', url: 'https://www.ffvbbeach.org/ffvbapp/resu/vbspo_calendrier.php?saison=2025%2F2026&codent=ABCCS&poule=3FB&division=&tour=&calend=COMPLET&x=20&y=18' },
  { code: 'r2m', championnatId: 'regionale-2-m', stateKeyClassement: 'r2m-classement', stateKeyMatchs: 'r2m-matchs', nom: 'Régionale 2 Masculine', url: 'https://www.ffvbbeach.org/ffvbapp/resu/vbspo_calendrier.php?saison=2025%2F2026&codent=LILR&poule=RM2&division=&tour=&calend=COMPLET' },
  { code: 'pnf', championnatId: 'prenationale-f', stateKeyClassement: 'pnf-classement', stateKeyMatchs: 'pnf-matchs', nom: 'Prénationale Féminine', url: 'https://www.ffvbbeach.org/ffvbapp/resu/vbspo_calendrier.php?saison=2025/2026&codent=LILR&poule=PFA' },
  { code: 'pnm', championnatId: 'prenationale-m', stateKeyClassement: 'pnm-classement', stateKeyMatchs: 'pnm-matchs', nom: 'Prénationale Masculine', url: 'https://www.ffvbbeach.org/ffvbapp/resu/vbspo_calendrier.php?saison=2025/2026&codent=LILR&poule=PMA' },
  { code: 'r2f', championnatId: 'regionale-2-f', stateKeyClassement: 'r2f-classement', stateKeyMatchs: 'r2f-matchs', nom: 'Régionale 2 Féminine', url: 'https://www.ffvbbeach.org/ffvbapp/resu/vbspo_calendrier.php?saison=2025%2F2026&codent=LILR&poule=RF2&division=&tour=&calend=COMPLET' },
];

const template = (championnatId: string, code: string, stateKeyClassement: string, stateKeyMatchs: string, nom: string, url: string) => `import { initializeApp } from 'firebase/app';
...
const CLASSEMENT_STATE_KEY = '${stateKeyClassement}';
const MATCHS_STATE_KEY = '${stateKeyMatchs}';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes ${nom} depuis Firebase...');
...
async function main() {
  ...
  const logger = initLogger('smart-update-${code}');
  console.log(\`📝 Logs enregistrés dans: \${logger.getLogFilePath()}\\n\`);

  try {
    console.log('🏐 Mise à jour SMART ${nom} (Classement + Matchs fusionnés)\\n');
    console.log('════════════════════════════════════════════════\\n');
...
console.log('🚀 Génération des scripts fusionnés pour les championnats adultes...\n');
...
for (const champ of championnats) {
  ...
  const content = template(champ.championnatId, champ.code, champ.stateKeyClassement, champ.stateKeyMatchs, champ.nom, champ.url);

  fs.writeFileSync(filePath, content, 'utf-8');
  console.log(`✅ ${fileName} créé`);
}

console.log('\n🎉 Tous les scripts fusionnés adultes ont été générés avec succès !');
