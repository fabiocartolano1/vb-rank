...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes BFC depuis Firebase...');
  const equipesQuery = query(
    collection(db, 'equipes'),
    where('championnatId', '==', 'benjamines-f-comite')
  );
...
async function scrapeMatchs(url: string, equipesMap: Map<string, string>): Promise<Match[]> {
  ...
  $('tr').each((_, element) => {
    ...
    const match: any = {
      championnatId: 'benjamines-f-comite',
...
async function main() {
  const logger = initLogger('smart-update-matchs-bfc');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART des Matchs Benjamines F Comité\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase
    const url = await getChampionnatUrl('benjamines-f-comite');
