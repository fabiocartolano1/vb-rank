...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes MFD depuis Firebase...');
  const equipesQuery = query(
    collection(db, 'equipes'),
    where('championnatId', '==', 'minimes-f-departement')
  );
...
async function scrapeMatchs(url: string, equipesMap: Map<string, string>): Promise<Match[]> {
  ...
  $('tr').each((_, element) => {
    ...
    const match: any = {
      championnatId: 'minimes-f-departement',
...
async function main() {
  const logger = initLogger('smart-update-matchs-mfd');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART des Matchs Minimes F Département\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase
    const url = await getChampionnatUrl('minimes-f-departement');
