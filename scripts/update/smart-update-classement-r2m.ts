...
const STATE_KEY = 'r2m-classement';
...
async function main() {
  const logger = initLogger('smart-update-classement-r2m');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART du Classement Régionale 2 Masculine\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    const url = 'https://www.ffvbbeach.org/ffvbapp/resu/vbspo_calendrier.php?saison=2025%2F2026&codent=LILR&poule=RM2&division=&tour=&calend=COMPLET';
