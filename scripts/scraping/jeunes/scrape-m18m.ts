...
async function main() {
  ...
  if (championnatDoc.empty) {
    console.error('❌ Championnat M18M non trouvé dans Firebase');
    process.exit(1);
  }
  ...
  if (!url) {
    console.error('❌ URL non renseignée pour M18M dans Firebase');
  ...
  try {
    ...
    console.log('\n✅ Scraping M18M terminé avec succès!\n');
    process.exit(0);
  } catch (error) {
    console.error('\n❌ Erreur:', error);
    process.exit(1);
