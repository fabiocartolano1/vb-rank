...
async function getEquipesMap(): Promise<Map<string, string>> {
  ...
  const equipesQuery = query(
    ...
    where('championnatId', '==', 'benjamins-m-bronze')
  );
...
async function scrapeMatchs(url: string, equipesMap: Map<string, string>): Promise<Match[]> {
  ...
  $('tr').each((_, element) => {
    ...
    const match: any = {
      championnatId: 'benjamins-m-bronze',
...
async function main() {
  const logger = initLogger('smart-update-matchs-bmb');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART des Matchs Benjamins M Bronze\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase
    const url = await getChampionnatUrl('benjamins-m-bronze');
