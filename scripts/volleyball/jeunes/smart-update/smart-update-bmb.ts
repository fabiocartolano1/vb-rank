...
const CLASSEMENT_STATE_KEY = 'bmb-classement';
const MATCHS_STATE_KEY = 'bmb-matchs';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes Benjamins Masculin depuis Firebase...');
  const equipesQuery = query(
    collection(db, 'equipes'),
    where('championnatId', '==', 'benjamins-m')
  );
...
async function main() {
  ...
  const logger = initLogger('smart-update-bmb');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Benjamins Masculin (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');
