...
const CLASSEMENT_STATE_KEY = 'cfd-classement';
const MATCHS_STATE_KEY = 'cfd-matchs';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  ...
  const equipesQuery = query(
    ...
    where('championnatId', '==', 'cadettes-f')
  );
...
async function main() {
  ...
  const logger = initLogger('smart-update-cfd');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Cadettes Féminin (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');
