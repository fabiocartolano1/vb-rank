...
const CLASSEMENT_STATE_KEY = 'mmb-classement';
const MATCHS_STATE_KEY = 'mmb-matchs';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes Minimes Masculin depuis Firebase...');
  const equipesQuery = query(
    collection(db, 'equipes'),
    where('championnatId', '==', 'minimes-m')
  );
...
async function main() {
  ...
  const logger = initLogger('smart-update-mmb');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Minimes Masculin (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase (1 seule fois)
    const url = await getChampionnatUrl('mmb');
