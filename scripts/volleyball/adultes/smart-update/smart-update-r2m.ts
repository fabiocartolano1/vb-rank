...
const CLASSEMENT_STATE_KEY = 'r2m-classement';
const MATCHS_STATE_KEY = 'r2m-matchs';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes Régionale 2 Masculine depuis Firebase...');
...
async function main() {
  ...
  const logger = initLogger('smart-update-r2m');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Régionale 2 Masculine (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase (1 seule fois)
    const url = await getChampionnatUrl('regionale-2-m');
