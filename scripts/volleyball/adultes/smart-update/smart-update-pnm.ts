...
const CLASSEMENT_STATE_KEY = 'pnm-classement';
const MATCHS_STATE_KEY = 'pnm-matchs';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes Prénationale Masculine depuis Firebase...');
...
async function updateEquipesInFirebase(equipes: EquipeData[], equipesMap: Map<string, string>): Promise<void> {
  ...
  for (const equipe of equipes) {
    ...
    if (equipeId) {
      ...
      const equipeSnap = await equipeRef.get();

      if (equipeSnap.exists) {
        const existingData = equipeSnap.data()!;
...
async function main() {
  ...
  const logger = initLogger('smart-update-pnm');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Prénationale Masculine (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase (1 seule fois)
    const url = await getChampionnatUrl('prenationale-m');
