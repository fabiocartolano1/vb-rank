...
const CLASSEMENT_STATE_KEY = 'n3-classement';
const MATCHS_STATE_KEY = 'n3-matchs';
...
async function main() {
  ...
  const logger = initLogger('smart-update-n3');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Nationale 3 (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase (1 seule fois)
    const url = await getChampionnatUrl('nationale-3-f');
