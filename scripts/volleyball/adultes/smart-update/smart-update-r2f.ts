...
const CLASSEMENT_STATE_KEY = 'r2f-classement';
const MATCHS_STATE_KEY = 'r2f-matchs';
...
async function getEquipesMap(): Promise<Map<string, string>> {
  console.log('📥 Récupération des équipes Régionale 2 Féminine depuis Firebase...');
...
async function updateEquipesInFirebase(equipes: EquipeData[], equipesMap: Map<string, string>): Promise<void> {
  ...
  for (const equipe of equipes) {
    ...
    if (equipeId) {
      ...
      const existingDoc = await equipeRef.get();
...
async function main() {
  ...
  const logger = initLogger('smart-update-r2f');
  console.log(`📝 Logs enregistrés dans: ${logger.getLogFilePath()}\n`);

  try {
    console.log('🏐 Mise à jour SMART Régionale 2 Féminine (Classement + Matchs fusionnés)\n');
    console.log('════════════════════════════════════════════════\n');

    await verifyEnvironment();

    // Récupérer l'URL depuis Firebase (1 seule fois)
    const url = await getChampionnatUrl('regionale-2-f');
